...
def autofocus_scan(start, stop, coarse_step, fine_range, fine_step, fit_top_n=5):
    ...
    best_pos = None
    try:
        df_fine_top = df_fine.sort_values(by="sharpness", ascending=False).head(fit_top_n)
    ...
    # --- Plot ---
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_coarse['position'], y=df_coarse['sharpness'],
                             mode='markers+lines', name='Coarse Scan'))
    fig.add_trace(go.Scatter(x=df_fine['position'], y=df_fine['sharpness'],
                             mode='markers+lines', name='Fine Scan'))
    fig.update_yaxes(insiderange=[0,50])

    # Overlay fit if successful
    try:
        if fit_label == "Quadratic":
            x_curve = np.linspace(df_fine['position'].min(), df_fine['position'].max(), 200)
            y_curve = np.poly1d(coeffs_fine)(x_curve)
            fig.add_trace(go.Scatter(x=x_curve, y=y_curve, mode='lines', name='Quadratic Fit'))
        elif fit_label == "Gaussian":
            x_curve = np.linspace(df_fine['position'].min(), df_fine['position'].max(), 200)
            y_curve = gaussian(x_curve, *popt)
            fig.add_trace(go.Scatter(x=x_curve, y=y_curve, mode='lines', name='Gaussian Fit'))
    except:
        pass

    fig.add_vline(x=best_pos, line_dash="dash", line_color="green", annotation_text=f"{fit_label} best: {best_pos:.3f}")
    fig.update_layout(title="Autofocus Coarse + Fine Scan", xaxis_title="Position", yaxis_title="Sharpness")
    # fig.show()
...
def define_plane_frame(reference_points):
    if 'H12' in reference_points:
        A1, H1, H12 = reference_points['A1'], reference_points['H1'], reference_points['H12']
        # Define axes
        u_axis = H12 - H1 # columns, x axis, alignment y, vertical in instrument space
        v_axis = H1 - A1  # rows, y axis, plate translation, horizontal in instrument space
        origin = A1

    else:
        A1, H1, A12 = reference_points['A1'], reference_points['H1'], reference_points['A12']
        # Define axes
...
def move_to_well_spot(well_input: str, plane: PlaneFrame, config: dict):
    ...
    if not match:
        raise ValueError(f"Invalid input: {well_input}. Format should be like 'A4 well 2'")
