...
class GenericClient(object):
    ...
    def movePlateToShelf(self, row, col, shelf, timeout=20):
        ...
        try:
            ...
            Logger.log('An error occurred while moving the plate.',
                       Logger.FAILED)
            if useEvents:
    ...
    def homeMotor(self, motor, timeout=20):
        ...
        try:
            ...
            Logger.log('An error occured while homing the motor {} : {}'.format(
                motor, e), Logger.FAILED)
            if useEvents:
        ...
        if self.retrieveTaskInfo(tid)[TASK_INFO_END_IDX] != "null":
            if useEvents:
                self.homeStartLock.release()
                self.taskLock.release()
            Logger.log("The %s motor homing did not start" %
                       motor, Logger.FAILED)
            return False
        if useEvents:
...
#            self.moveLock.release()
            ...
            while self.getMotorState(motor).casefold() != "Initializing".casefold() and time.process_time() - start < maxHomingTime:
                pass
        while len(self.monitoredHomingMotors[mName]) == 0 and time.process_time() - start < maxHomingTime:
            pass
        if len(self.monitoredHomingMotors[mName]) == 0:
            Logger.log("The %s motor should be initializing, while state is %s." % (
                motor, self.getMotorState(motor)), Logger.FAILED)
        else:
            Logger.log("The %s motor homing was triggered." %
                       (motor), Logger.OK)
        ...
        if self.getMotorState(motor) != "Ready":
            Logger.log("The %s motor homing failed, state was %s" %
                       (motor, self.getMotorState(motor)), Logger.FAILED)
        else:
    ...
    def waitAndCheck(self, task_name, id, cmd_start, expected_time, timeout):
        ...
        if act_time >= timeout:
            Logger.log("%s %d failed due to Timeout" %
                       (task_name, id), success=Logger.FAILED)
            return False
            ...
            Logger.log("%s %d passed in %.3f sec " %
                       (task_name, id, act_time), success=Logger.OK)
            return True
    ...
    def waitAppReady(self, cmd_start, expected_time, timeout):
        ...
        while True:
            ...
            real_time = time.perf_counter() - cmd_start
            # print(server.State)
            if str.lower(str(self['State'])) == "ready" or str.lower(str(self['State'])) == "on":
                break
            if real_time > timeout:
                Logger.log("Server Timeout", success='DEBUG')
                break
    ...
    def syncMoveMotor(self, motor, position, timeout=30, use_events=True):
        ...
        try:
            ...
            Logger.log("The %s motor could not move due to an error %s." %
                       (motor, e), Logger.FAILED)
            if use_events and self.hasEvents():
        ...
        if use_events and self.hasEvents():
            ...
            while motorState != "Ready" and motorState != "LowLim" and motorState != "HighLim":
                motorState = self[motorStateAttribute]
                time.sleep(0.1)
                timeoutExpired = (time.perf_counter() -
                                  pollingStartClock) > timeout
                if timeoutExpired:
                    raise Exception(
                        "Timeout occurred while moving motor %s to position %.4f" % (motor, position))
                pass
    ...
    def moveAndWaitEndOfMove(self, motor, initialPos, position, useAttr, n, useEvents=False, goBack=False, timeout=20, backMove=False):
        ...
        if abs(initialPos - float(self.getMotorPosition(motor))) > 0.01:
            self.moveAndWaitEndOfMove(motor, float(self.getMotorPosition(
                motor)), initialPos, useAttr, 1, useEvents, False, timeout)
        try:
            expectedMoveTime = max(0.1, float(
                self.getMotorMoveTime(motor, position)))
            Logger.trace("The %s motor will move from %.4f to %.4f in %.4f s" % (
                motor, initialPos, position, expectedMoveTime))
        except:
            expectedMoveTime = 0.1
            Logger.trace("The %s motor will move from %.4f to %.4f in unknown time " % (
        ...
        for i in range(0, n):
            ...
            try:
                ...
                Logger.log("The %s motor could not move due to an error %s. %s" % (
                    motor, e, pass_log), Logger.FAILED)
                if useEvents:
            ...
            if expectedMoveTime > 1:
                ...
                if str(st).casefold() != "Moving".casefold() and len(self.monitoredMotors[mName]) < 2:
                    Logger.log("The %s motor should be moving, while state is %s. %s" % (
                        motor, st, pass_log), Logger.FAILED)
                else:
                    Logger.log("The %s motor movement was triggered. %s" %
                               (motor, pass_log), Logger.OK)

            if useEvents:
                Logger.trace("Waiting for events, timeout = %.4f + %d sec" %
                ...
                if len(self.monitoredMotors[mName]) != 2:
                    ...
                    realMoveTime = (
                        self.monitoredMotors[mName][1][0] - self.monitoredMotors[mName][0][0])/1000.
            ...
            if realMoveTime == -1:
                ...
                log_msg = "Motor %s: %.4f s elapsed for the movement, " \
                          ...
                          (motor, realMoveTime, expectedMoveTime,
                           (realMoveTime-expectedMoveTime), pass_log)
    ...
    def isMotorStateConsistant(self, motor):
        try:
            ...
            Logger.log('Error reading {} state, via attribute {} -> {}'.format(motor,
                       att_name, att), Logger.FAILED)
            return
        try:
            meth = self.getMotorState(motor)
        except Exception as e:
            Logger.log('Error reading {} state, via method call.'.format(
                motor), Logger.FAILED)
            return
        if meth.lower() != att.lower():
            Logger.log('Inconsistency while reading {} state. {} attribute reading : {} \n Method reading : {}.'.format(
                motor, att_name, meth), Logger.FAILED)
        else:
            Logger.log('Attribute and method state for motor {} are consistant'.format(
                motor), Logger.OK)
    ...
    def isMotorPositionConsistant(self, motor):
        try:
            ...
            Logger.log('Error reading {} state, via attribute {}'.format(
                motor, ''.join([motor.replace(" ", ""), "Position"])), Logger.FAILED)
            return
        try:
        ...
        if (meth - att) > 0.1:
            Logger.log('Inconsistency while reading {} position. {} attribute reading : {} \n Method reading : {}.'.format(
                motor, ''.join([motor.replace(" ", ""), "Position"]), att, meth), Logger.FAILED)
        else:
            Logger.log('Attribute and method position for motor {} are consistent'.format(
                motor), Logger.OK)
        ...
        for idx, attr in enumerate(attributes):
            try:
                ...
                if type(attrVal) is list and len(attrVal) > 1000:
                    # picture so print it
                    if DISPLAY_IMAGE:
                        Image.open(StringIO(struct.pack(
                            'b' * len(attrVal), *attrVal))).show()
                        Logger.log('Reading attribute %s' %
                                   (attr.name), Logger.OK)
                else:
                    Logger.log('Reading attribute %s value = %s' %
                               (attr.name, attrVal), Logger.OK)
                if attr.writable is False:
                    tmp = presentRWAttributes.pop(idx+offset)
                    offset -= 1
            except Exception as e:
                Logger.log('Failed to read attribute %s error : %s' %
                           (attr.name, e), Logger.FAILED)
    ...
    def testPredefinedPositions(self, device, positions, useEvents=lambda self: self.hasEvents(), goBack=False, timeout=30):
        ...
        for pos in positions:
            lastPosition = self.setPredefinedPosition(
                device, pos, useEvents, timeout)
        ...
        if initial_position is None or initial_position.casefold() == "Unknown".casefold()\
            ...
            try:
                ...
                if useEvents:
                    ...
                    self.moveLock.release()
                Logger.log("Could not move {} to position {} : exception reading {} = {}.".format(
                    device, position, attr_pos_name, e), Logger.FAILED)
            ...
            if not self.isDeviceInPosition(device, position):
                Logger.log("Could not move {} to exact position {} : Found={}.".format(
                    device, position, self[attr_pos_name]), Logger.FAILED)
            else:
                Logger.log('{} device went into position {}.'.format(
                    device, position), Logger.OK)
                return position
        else:
            Logger.log("Device {} already into position {}.".format(
                device, position))
    ...
    def changePhase(self, position, expectedPos, useEvents=False, timeout=120):
        ...
        try:
            ...
            Logger.log('An error occurred while setting the phase {} : {}'.format(
                position, e), Logger.FAILED)
        ...
        if self['CurrentPhase'] != position:
            ...
            if tid in self.taskErrors:
                timeoutMsg = ". Exception occurred : {}".format(
                    self.taskErrors[tid])
                del self.taskErrors[tid]
            Logger.log('Failed to go to position {}{}.'.format(
                position, timeoutMsg), Logger.FAILED)
            return False
        else:
            Logger.log('Phase changed to position {} in {:.3f} sec'.format(
                position, duration), Logger.OK)
        ...
        for key, value in iter(expectedPos.items()):
            ...
            while not self.isDeviceInPosition(key, value) and time.process_time() - devWaitStartTime < 10:
                pass
            if self.isDeviceInPosition(key, value):
                Logger.log('Device {} is in position {}.'.format(
                    key, value), Logger.OK)
            else:
                Logger.log('Device {} is in position {}. {} expected.'.format(
                    key, self[''.join([key, 'Position'])], value), Logger.FAILED)

        self.waitAppReady(time.perf_counter(), 60, 120)
        duration = time.process_time() - sTime
        Logger.log('App Ready after Phase changed to position {} in {:.3f} sec'.format(
            position, duration), Logger.OK)
        return True
    ...
    def toggleIO(self, io, useEvents=lambda self: self.hasEvents(), timeout=10):
        ...
        try:
            ...
            Logger.log('An exception occurred for IO {} : {}.'.format(
                io, e), Logger.NA)
            return
        if self[io] == state:
            Logger.log('IO {} cannot be set to {}.'.format(
                io, not state), Logger.FAILED)
        else:
    ...
    def monitorTaskInfo(self, taskId, useEvents=lambda self: self.hasEvents()):
        ...
        # print("tInfo", tInfo)
        if tInfo is None:
            Logger.log("Task with ID %s has no task info." %
                       str(taskId), Logger.FAILED)
