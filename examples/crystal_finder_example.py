...
crystals_flat = crystal_finder.plot_crystal_finder_results(save=True, filename="flat")

# Run the crystal finder for the second grid scan
crystal_finder = CrystalFinder(number_of_spots=number_of_spots, threshold=5)
crystals_edge = crystal_finder.plot_crystal_finder_results(save=True, filename="edge")

# Find the crystals volumes
crystal_finder_3d = CrystalFinder3D(
    crystals_flat=crystals_flat,
    crystals_edge=crystals_edge,
)
