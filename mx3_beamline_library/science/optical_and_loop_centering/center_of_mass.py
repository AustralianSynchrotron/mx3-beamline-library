...
def multi_crystal_center_of_mass(number_of_spots: npt.NDArray, threshold: int) -> list[tuple[int,int]]:
    """
...
def plot_muti_crystal_center_of_mass(number_of_spots: npt.NDArray, threshold: int, save: bool=False):
    """
    Calculates the center of mass of individual crystals given an 
    ...
    for center_of_mass in center_of_mass_list:
        ...
        plt.scatter(center_of_mass[0], center_of_mass[1], label="CM", marker="+", s=200, color="red")
...
if __name__ == "__main__":
    ...
    new = np.append(number_of_spots,np.array([0, 0, 0]))
    rotated_array = np.rot90(np.append(new, number_of_spots).reshape(7,3), k=1)
