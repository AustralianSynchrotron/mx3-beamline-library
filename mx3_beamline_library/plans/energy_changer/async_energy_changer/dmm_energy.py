...
class DMMEnergy:
    ...
    async def _set_bragg_angle(self, energy: float, bragg_angle_offset: float) -> float:
        """
    ...
    async def _set_parallel_translation(self, bragg_angle: float) -> None:
        ...
        self.parallel_translation_motor.set(R)
        await self._wait(self.parallel_translation_motor)

    async def set_dmm_energy(self, energy: float, bragg_angle_offset: float) -> None:
        """
        ...
        None
        """
        bragg_angle = await self._set_bragg_angle(energy, bragg_angle_offset)
        await self._set_parallel_translation(bragg_angle)
